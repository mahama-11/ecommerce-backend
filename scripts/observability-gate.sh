...
status=0
go test ./internal/observability ./internal/telemetry ./internal/middleware -count=1 -cover 2>&1 | tee "$LOG" || status=${PIPESTATUS[0]}
...
required = ['ecommerce-service/internal/observability','ecommerce-service/internal/telemetry','ecommerce-service/internal/middleware']
failures = []
