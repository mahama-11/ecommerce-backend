...
func TestClientWithContextPropagatesRequestTraceAndTraceparentToPublicPlatformCalls(t *testing.T) {
	seen := map[string]string{}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen["request_id"] = r.Header.Get("X-Request-ID")
		seen["trace_id"] = r.Header.Get("X-Trace-ID")
		seen["traceparent"] = r.Header.Get("traceparent")
		seen["internal_service"] = r.Header.Get("X-Internal-Service")
		writeEnvelope(t, w, http.StatusOK, 0, map[string]any{"access_token": "token", "user": map[string]any{"id": "user-ctx", "email": "owner@example.test", "org_id": "org-ctx", "status": "active"}}, "", "", "")
	}))
	defer server.Close()
	...
	if seen["request_id"] != "req-ecommerce-public" {
		t.Fatalf("X-Request-ID=%q", seen["request_id"])
	}
	if seen["trace_id"] != traceID.String() {
		t.Fatalf("X-Trace-ID=%q", seen["trace_id"])
	...
	if seen["internal_service"] != "" {
		t.Fatalf("public call should not set X-Internal-Service, got %q", seen["internal_service"])
	}
}
