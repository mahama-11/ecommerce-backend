...
cd "$ROOT_DIR"
REPORT_DIR=${INFRA_CROSSCUT_REPORT_DIR:-reports/quality/infra-crosscut}
mkdir -p "$REPORT_DIR"
LOG="$REPORT_DIR/go-test-infra-crosscut.log"
REPORT="$REPORT_DIR/latest.json"
status=0
go test ./internal/router ./internal/middleware ./internal/storage ./internal/config ./internal/app ./internal/migration -count=1 -cover 2>&1 | tee "$LOG" || status=${PIPESTATUS[0]}
...
required = ['ecommerce-service/internal/router','ecommerce-service/internal/middleware','ecommerce-service/internal/storage','ecommerce-service/internal/config','ecommerce-service/internal/app','ecommerce-service/internal/migration']
failures = []
